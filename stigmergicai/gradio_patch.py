...
from agency_swarm.shared_state import SharedState
from openai.types.beta.threads.runs.tool_call import ToolCall, FunctionCall, CodeInterpreterCall, FileSearchToolCall
...
def demo_gradio(self, height=450, dark_mode=True, **kwargs):
        ...
        def instruction():
            sender = None
            for s in self.agents_and_threads.keys():
                if recipient_agent.name in self.agents_and_threads[s].keys():
                    sender = s
                    break

            assert sender is not None
            thread = self.agents_and_threads[sender][recipient_agent.name]
            res = thread.get_completion(message= 'What is your current understanding about the instuction?')
        ...
        with gr.Blocks(js=js) as demo:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Interface(fn=instruction, input=[], outputs= gr.Textbox(label="Current Instruction", lines= 3, max_lines=3), allow_flagging="never")
