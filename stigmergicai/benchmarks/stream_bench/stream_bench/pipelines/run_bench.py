...
def setup_args() -> Namespace:
    ...
    parser.add_argument(
        "--project", type=str, default=None, help="Project name for wandb"
    )
    parser.add_argument("--entity", type=str, default=None, help="Team name for wandb")
    parser.add_argument(
        "--name", type=str, default=None, help="The name of the experiment for wandb."
    )
...
def main():
    ...
    if args.use_wandb:
        wandb.init(
            ...
            else f"streambench-{bench_cfg['bench_name']}",
            entity=args.entity,
            name=args.name if (args.name is not None) else agent.get_name(),
            config=merge_dicts(
                dicts=[agent_cfg, bench_cfg]
            ),  # NOTE: agent configurations and benchmark configurations
        )
    ...
    for time_step, row in enumerate(tqdm(bench.get_dataset(), dynamic_ncols=True)):
        try:
            ...
            pred_res = bench.process_results(
                prediction, label, return_details=True, time_step=time_step
            )
