from __future__ import annotations

from pathlib import Path

from .config import load_bundle_config, load_output_schema
from .schemas import Bundle, BundleConfig, OutputSchema


def test_bundle_requires_minimal_fields() -> None:
    bundle = Bundle(
        field_name="example",
        format="freeform",
        most_current_data={"k": "v"},
    )
    assert bundle.field_name == "example"
    assert bundle.format == "freeform"
    assert bundle.most_current_data == {"k": "v"}


def test_bundle_rejects_invalid_format() -> None:
    from pydantic import ValidationError

    try:
        Bundle(field_name="x", format="unknown", most_current_data={})
        assert False, "Expected ValidationError for invalid format"
    except ValidationError:
        # expected
        pass


def test_load_bundle_config_works(tmp_path: Path) -> None:
    # write a small YAML bundle_config to a temp file to exercise extension switch
    yaml_path = tmp_path / "bundle_config.yaml"
    yaml_path.write_text(
        "bundle_order:\n"
        "  - field_name: foo\n"
        "    section_title: Section Foo\n"
        "    order: 0\n",
        encoding="utf-8",
    )
    cfg: BundleConfig = load_bundle_config(yaml_path)
    assert len(cfg.bundle_order) == 1
    assert cfg.bundle_order[0].field_name == "foo"
    assert cfg.bundle_order[0].section_title == "Section Foo"
    assert cfg.bundle_order[0].order == 0


def test_load_output_schema_builtin() -> None:
    # use the real output_schema.json in this package
    here = Path(__file__).parent
    schema_path = here / "output_schema.json"
    schema: OutputSchema = load_output_schema(schema_path)
    # at minimum, title should be parsed
    assert isinstance(schema.title, str)

