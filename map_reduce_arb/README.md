...
A bundle is intentionally minimal:
- `field_name`: section identifier (e.g., `"collateral"`, `"business_model"`)
- `prompt`: prompt text to apply for the section. If it equals the sentinel `__NONE__`, the section is **passed through** without calling the LLM.
- `payload`: string payload; typically a JSON string for the LLM path, or markdown table text for passthrough
...
**Note:** The map-reduce graph now runs `enrich_transform` first, then builds two bundles (`collateral`, `business_model`) from the enriched result.
